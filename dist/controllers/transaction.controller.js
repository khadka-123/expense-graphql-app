...
const getTransaction = catchAsync(async (_, args) => {
    const transactions = await transactionModel.find({
        userId: args.userId
    });
    return transactions;
});
const addTransaction = catchAsync(async (_, args) => {
...
const updateTransaction = catchAsync(async (_, args) => {
    const { userId, transactionId, input } = args;
    const updated = await transactionModel.findOneAndUpdate({ _id: transactionId, userId }, //filter
    input //update
    );
    if (!updated)
...
const deleteTransaction = catchAsync(async (_, args) => {
